...
const Login = () => {
  ...
  const handleLogin = (e) => {
   ...
   return (
      <div className="min-h-screen w-full bg-gradient-to-br from-slate-50 to-slate-100 px-3 py-6 font-inter antialiased">
      <div className="grid w-full max-w-[1200px] gap-6 xl:grid-cols-[280px_minmax(0,1fr)] mx-auto">
        {/* Left Section - Professional Branding (Compact) */}
      <div className="hidden xl:flex xl:w-[280px] 2xl:w-[300px] relative bg-[#CAD3C8] overflow-hidden rounded-[20px] border border-slate-200 shadow-sm">
      ...
      <div className="relative flex min-h-[440px] w-full items-center justify-center bg-transparent border-none overflow-visible px-2">
        <AnimatePresence mode="wait">
          {!selectedRole ? (
                     ...
                     className="w-full max-w-[820px] rounded-[18px] bg-white border border-slate-100 shadow-md p-8 lg:p-10 mx-auto"
                  >
               <div className="mb-8 text-center">
                  <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight mb-1">Command Portals</h2>
                  <p className="text-slate-500 text-[9px] font-semibold uppercase tracking-widest">Select Node for Authentication</p>
               </div>
                   ...
                   {ROLES.map((role, idx) => (
                     <motion.button
                       ...
                       onClick={() => setSelectedRole(role)}
                                 className="bg-white p-4 rounded-[14px] border border-slate-100 shadow-sm hover:shadow-md hover:border-slate-200 transition-all group flex flex-col items-center justify-center text-center relative min-h-[96px]"
                                               >
                                               <div className={`${role.color} group-hover:scale-115 transition-transform mb-2`}>
              ...
              exit={{ opacity: 0, x: -20 }}
                     className="w-full max-w-[460px] rounded-[20px] bg-slate-50 border border-slate-200 shadow-sm p-6 lg:p-7 mx-auto"
            >
               <button 
               ...
               <div className="mb-6 text-center">
                  <div className={`w-14 h-14 ${selectedRole.bg} ${selectedRole.color} rounded-xl flex items-center justify-center mb-4 border border-black/5`}>
                     <selectedRole.icon size={24} />
                  </div>
                  <h2 className="text-xl font-black text-slate-900 tracking-tighter uppercase leading-none">{selectedRole.name}</h2>
                  <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mt-2">Enter Command Credentials</p>
               </div>
